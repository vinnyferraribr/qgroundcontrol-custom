...
QGC_LOGGING_CATEGORY(TubetesComplexItemLog, "TubetesComplexItemLog")

const QString TubetesComplexItem::name(TubetesComplexItem::tr("Tubetes"));

TubetesComplexItem::TubetesComplexItem(PlanMasterController* masterController, bool flyView, const QString& kmlOrShpFile)
      ...
      , _entryPoint               (EntryLocationTopLeft)
{
    ...
    connect(&_gridAngleFact,            &Fact::valueChanged,                        this, &TubetesComplexItem::_setDirty);
    connect(&_flyAlternateTransectsFact,&Fact::valueChanged,                        this, &TubetesComplexItem::_setDirty);
    connect(&_splitConcavePolygonsFact, &Fact::valueChanged,                        this, &TubetesComplexItem::_setDirty);
    connect(this,                       &TubetesComplexItem::refly90DegreesChanged, this, &TubetesComplexItem::_setDirty);

    connect(&_gridAngleFact,            &Fact::valueChanged,                        this, &TubetesComplexItem::_rebuildTransects);
    connect(&_flyAlternateTransectsFact,&Fact::valueChanged,                        this, &TubetesComplexItem::_rebuildTransects);
    connect(&_splitConcavePolygonsFact, &Fact::valueChanged,                        this, &TubetesComplexItem::_rebuildTransects);
    connect(this,                       &TubetesComplexItem::refly90DegreesChanged, this, &TubetesComplexItem::_rebuildTransects);

    connect(&_surveyAreaPolygon,        &QGCMapPolygon::isValidChanged,             this, &TubetesComplexItem::_updateWizardMode);
    connect(&_surveyAreaPolygon,        &QGCMapPolygon::traceModeChanged,           this, &TubetesComplexItem::_updateWizardMode);
...
void TubetesComplexItem::save(QJsonArray&  planItems)
{
...
void TubetesComplexItem::savePreset(const QString& name)
{
    QJsonObject saveObject;
...
void TubetesComplexItem::_saveCommon(QJsonObject& saveObject)
{
...
void TubetesComplexItem::loadPreset(const QString& name)
{
    QString errorString;
...
bool TubetesComplexItem::load(const QJsonObject& complexObject, int sequenceNumber, QString& errorString)
{
...
bool TubetesComplexItem::_loadV4V5(const QJsonObject& complexObject, int sequenceNumber, QString& errorString, int version, bool forPresets)
{
...
bool TubetesComplexItem::_loadV3(const QJsonObject& complexObject, int sequenceNumber, QString& errorString)
{
...
void TubetesComplexItem::_reverseTransectOrder(QList<QList<QGeoCoordinate>>& transects)
{
...
void TubetesComplexItem::_reverseInternalTransectPoints(QList<QList<QGeoCoordinate>>& transects)
{
...
void TubetesComplexItem::_optimizeTransectsForShortestDistance(const QGeoCoordinate& distanceCoord, QList<QList<QGeoCoordinate>>& transects)
{
...
qreal TubetesComplexItem::_ccw(QPointF pt1, QPointF pt2, QPointF pt3)
{
    return (pt2.x()-pt1.x())*(pt3.y()-pt1.y()) - (pt2.y()-pt1.y())*(pt3.x()-pt1.x());
}

qreal TubetesComplexItem::_dp(QPointF pt1, QPointF pt2)
{
    return (pt2.x()-pt1.x())/qSqrt((pt2.x()-pt1.x())*(pt2.x()-pt1.x()) + (pt2.y()-pt1.y())*(pt2.y()-pt1.y()));
}

void TubetesComplexItem::_swapPoints(QList<QPointF>& points, int index1, int index2)
{
...
bool TubetesComplexItem::_gridAngleIsNorthSouthTransects()
{
...
void TubetesComplexItem::_adjustTransectsToEntryPointLocation(QList<QList<QGeoCoordinate>>& transects)
{
    ...
    if (reversePoints) {
        qCDebug(TubetesComplexItemLog) << "_adjustTransectsToEntryPointLocation Reverse Points";
        _reverseInternalTransectPoints(transects);
    }
    if (reverseTransects) {
        qCDebug(TubetesComplexItemLog) << "_adjustTransectsToEntryPointLocation Reverse Transects";
        _reverseTransectOrder(transects);
    }

    qCDebug(TubetesComplexItemLog) << "_adjustTransectsToEntryPointLocation Modified entry point:entryLocation" << transects.first().first() << _entryPoint;
}

QPointF TubetesComplexItem::_rotatePoint(const QPointF& point, const QPointF& origin, double angle)
{
    QPointF rotated;
...
void TubetesComplexItem::_intersectLinesWithRect(const QList<QLineF>& lineList, const QRectF& boundRect, QList<QLineF>& resultLines)
{
...
void TubetesComplexItem::_intersectLinesWithPolygon(const QList<QLineF>& lineList, const QPolygonF& polygon, QList<QLineF>& resultLines)
{
    resultLines.clear();
...
void TubetesComplexItem::_adjustLineDirection(const QList<QLineF>& lineList, QList<QLineF>& resultLines)
{
    qreal firstAngle = 0;
    for (int i=0; i<lineList.count(); i++) {
        const QLineF& line = lineList[i];
...
double TubetesComplexItem::_clampGridAngle90(double gridAngle)
{
...
bool TubetesComplexItem::_nextTransectCoord(const QList<QGeoCoordinate>& transectPoints, int pointIndex, QGeoCoordinate& coord)
{
...
bool TubetesComplexItem::_hasTurnaround(void) const
{
    return _turnAroundDistance() > 0;
}

double TubetesComplexItem::_turnaroundDistance(void) const
{
    return _turnAroundDistanceFact.rawValue().toDouble();
}

void TubetesComplexItem::_rebuildTransectsPhase1(void)
{
...
void TubetesComplexItem::_rebuildTransectsPhase1WorkerSinglePolygon(bool refly)
{
    if (_ignoreRecalc) {
        return;
    }
    ...
    qCDebug(TubetesComplexItemLog) << "_rebuildTransectsPhase1 Convert polygon to NED - _surveyAreaPolygon.count():tangentOrigin" << _surveyAreaPolygon.count() << tangentOrigin;
    for (int i=0; i<_surveyAreaPolygon.count(); i++) {
        ...
        qCDebug(TubetesComplexItemLog) << "_rebuildTransectsPhase1 vertex:x:y" << vertex << polygonPoints.last().x() << polygonPoints.last().y();
    }
    ...
    qCDebug(TubetesComplexItemLog) << "_rebuildTransectsPhase1 Clamped grid angle" << gridAngle;

    qCDebug(TubetesComplexItemLog) << "_rebuildTransectsPhase1 gridSpacing:gridAngle:refly" << gridSpacing << gridAngle << refly;

            // Convert polygon to bounding rect

    qCDebug(TubetesComplexItemLog) << "_rebuildTransectsPhase1 Polygon";
    QPolygonF polygon;
    for (int i=0; i<polygonPoints.count(); i++) {
        qCDebug(TubetesComplexItemLog) << "Vertex" << polygonPoints[i];
        polygon << polygonPoints[i];
    }
    polygon << polygonPoints[0];
    QRectF boundingRect = polygon.boundingRect();
    QPointF boundingCenter = boundingRect.center();
    qCDebug(TubetesComplexItemLog) << "Bounding rect" << boundingRect.topLeft().x() << boundingRect.topLeft().y() << boundingRect.bottomRight().x() << boundingRect.bottomRight().y();
    ...
    for (const QList<QGeoCoordinate>& transect : transects) {
        ...
        if (triggerCamera() && hoverAndCaptureEnabled()) {
            ...
            if (triggerDistance() < transectLength) {
                ...
                qCDebug(TubetesComplexItemLog) << "cInnerHoverPoints" << cInnerHoverPoints;
...
void TubetesComplexItem::_rebuildTransectsPhase1WorkerSplitPolygons(bool refly)
{
    if (_ignoreRecalc) {
        return;
    }
...
//        qCDebug(TubetesComplexItemLog) << "Transects from polynom p " << p;
        _rebuildTransectsFromPolygon(refly, *p, tangentOrigin, vMatch);
    }
}

void TubetesComplexItem::_PolygonDecomposeConvex(const QPolygonF& polygon, QList<QPolygonF>& decomposedPolygons)
{
...
bool TubetesComplexItem::_VertexCanSeeOther(const QPolygonF& polygon, const QPointF* vertexA, const QPointF* vertexB) {
...
//    qCDebug(TubetesComplexItemLog) << "_VertexCanSeeOther false after first checks ";
...
//    qCDebug(TubetesComplexItemLog) << "_VertexCanSeeOther distanceAB " << distanceAB;
...
//            auto distanceIntersection = sqrtf(diffIntersection.x() * diffIntersection.x() + diff.y()*diff.y());
//            qCDebug(TubetesComplexItemLog) << "*vertexA " << *vertexA << "*vertexB " << *vertexB  << " intersection " << intersection;
            ...
            qCDebug(TubetesComplexItemLog) << "_VertexCanSeeOther distanceIntersection " << distanceIntersection;
...
bool TubetesComplexItem::_VertexIsReflex(const QPolygonF& polygon, QList<QPointF>::const_iterator& vertexIter) {
    auto vertexBefore = vertex == polygon.begin() ? polygon.end() - 1 : vertex - 1;
    auto vertexAfter = vertex == polygon.end() - 1 ? polygon.begin() : vertex + 1;
...
void TubetesComplexItem::_rebuildTransectsFromPolygon(bool refly, const QPolygonF& polygon, const QGeoCoordinate& tangentOrigin, const QPointF* const transitionPoint)
{
    // Generate transects
    ...
    qCDebug(TubetesComplexItemLog) << "_transects.size() " << _transects.size();
}

void TubetesComplexItem::_recalcCameraShots(void)
{
...
TubetesComplexItem::ReadyForSaveState TubetesComplexItem::readyForSaveState(void) const
{
    return TransectStyleComplexItem::readyForSaveState();
}

void TubetesComplexItem::rotateEntryPoint(void)
{
...
double TubetesComplexItem::timeBetweenShots(void)
{
    return _vehicleSpeed == 0 ? 0 : triggerDistance() / _vehicleSpeed;
}

double TubetesComplexItem::additionalTimeDelay (void) const
{
    double hoverTime = 0;
...
void TubetesComplexItem::_updateWizardMode(void)
{
